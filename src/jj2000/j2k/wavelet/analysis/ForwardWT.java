 ...
 * CVS identifier:
 *
 * $Id: ForwardWT.java,v 1.1 2006/12/14 20:59:36 devel Exp vidal $
 *
 * Class:                   ForwardWT
 *
...
public abstract class ForwardWT extends ImgDataAdapter
     ...
     * or if invalid values are given.
     * */
    public static ForwardWT createAltInstance(BlkImgDataSrc src,EncoderOptions options,
        ...
        // Code-block partition origin       
        int cb0x = options.wcboffx;        
        if(cb0x<0 || cb0x>1) {
            throw new IllegalArgumentException("Invalid horizontal "+
                                               "code-block partition origin.");
        }
        int cb0y = options.wcboffy;         
        if(cb0y<0 || cb0y>1) {
            throw new IllegalArgumentException("Invalid vertical "+
                                               "code-block partition origin.");
        }
        ...
        return new AltForwWTFull(src,encSpec,cb0x,cb0y);
    }
