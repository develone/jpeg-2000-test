 ...
 * CVS identifier:
 *
 * $Id: AltFileBitstreamReaderAgent.java,v 1.1 2006/12/14 20:59:05 devel Exp vidal $
 *
 ...
 * @see PktDecoder
 * */
public class AltFileBitstreamReaderAgent extends AltBitstreamReaderAgent
     ...
     * @see #allocateRate
     * */
    public AltFileBitstreamReaderAgent(AltHeaderDecoder hd,RandomAccessIO ehs,
            DecoderSpecs decSpec,DecoderOptions options,
            HeaderInfo hi)
            throws IOException {
        super(hd,decSpec);
        this.hi = hi;
        ...
        usePOCQuit = options.poc_quit;
        
        // Get decoding rate
        boolean rateInBytes;
        boolean parsing = options.parsing;
        trate = options.rate;
        ...
        tnbytes = options.nbytes;
        rateInBytes = tnbytes != -1;
        ...
        isTruncMode = !options.parsing;
        
        // Check if quit conditions are being used
        int ncbQuit = options.ncb_quit;
        ...
        lQuit = options.l_quit;
        
        // initializations
        in = ehs;
        pktDec = new AltPktDecoder(decSpec,hd,ehs,this,isTruncMode,ncbQuit);
        ...
        try {
            ...
            if(options.res_lev == -1) {
                targetRes = decSpec.dls.getMin();
            } else {
                targetRes = options.res_lev;
                if(targetRes<0) {
                    throw new
