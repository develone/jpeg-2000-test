 ...
 * CVS identifier:
 *
 * $Id: AltHeaderEncoder.java,v 1.1 2006/12/14 20:59:05 devel Exp vidal $
 *
 * Class:                   HeaderEncoder
 *
 ...
 * */
public class AltHeaderEncoder implements Markers, StdEntropyCoderOptions {
    ...
    protected AltPostCompRateAllocator ralloc;
     ...
     * */
    public AltHeaderEncoder(ImgData origsrc, boolean isorigsig[],
                         ForwardWT dwt, Tiler tiler,AltEncoderSpecs encSpec, 
			 FastROIScaler roiSc, AltPostCompRateAllocator ralloc) {
	...
	if(mh) {
            if( ((Boolean)encSpec.sops.getDefault()).booleanValue()) {
                tmp |= SCOX_USE_SOP;
            }
        } else {
            if( ((Boolean)encSpec.sops.getTileDef(tileIdx)).booleanValue() ){
                tmp |= SCOX_USE_SOP;
            }
        }
        ...
        if(mh) {
            if ( ((Boolean)encSpec.ephs.getDefault()).booleanValue()) {
                tmp |= SCOX_USE_EPH;
            }
        } else {
            if ( ((Boolean)encSpec.ephs.getTileDef(tileIdx) ).booleanValue()) {
                tmp |= SCOX_USE_EPH;
            }
        }
	...
	if(mh) { // Main header
	    ...
	    if( ((Boolean)encSpec.bms.getDefault()).booleanValue()) {
		tmp |= OPT_BYPASS;
	    }
	    // MQ reset after each coding pass ?
	    if( ((Boolean)encSpec.mqrs.getDefault()).booleanValue()) {
		tmp |= OPT_RESET_MQ;
	    }
	    // MQ termination after each arithmetically coded coding pass ?
	    if(  ((Boolean)encSpec.rts.getDefault()).booleanValue() ) {
		tmp |= OPT_TERM_PASS;
	    }
	    // Vertically stripe-causal context mode ?
	    if(  ((Boolean)encSpec.css.getDefault()).booleanValue() ) {
		tmp |= OPT_VERT_STR_CAUSAL;
	    }
            // Predictable termination ?
            if( ((Integer)encSpec.tts.getDefault()).intValue() == MQCoder.TERM_PRED_ER) {
                tmp |= OPT_PRED_TERM;
            }
	    // Error resilience segmentation symbol insertion ?
	    if(  ((Boolean)encSpec.sss.getDefault()).booleanValue()) {
		tmp |= OPT_SEG_SYMBOLS;
	    }
	} else { // Tile header
	    // Selective arithmetic coding bypass ?
	    if( ((Boolean)encSpec.bms.getTileDef(tileIdx)).booleanValue()) {
		tmp |= OPT_BYPASS;
	    }
	    // MQ reset after each coding pass ?
	    if( ((Boolean)encSpec.mqrs.getTileDef(tileIdx)).booleanValue()) {
		tmp |= OPT_RESET_MQ;
	    }
	    // MQ termination after each arithmetically coded coding pass ?
	    if(  ((Boolean)encSpec.rts.getTileDef(tileIdx)).booleanValue() ) {
		tmp |= OPT_TERM_PASS;
	    }
	    // Vertically stripe-causal context mode ?
	    if(  ((Boolean)encSpec.css.getTileDef(tileIdx)).booleanValue() ) {
		tmp |= OPT_VERT_STR_CAUSAL;
	    }
            // Predictable termination ?
            if( ((Integer)encSpec.tts.getTileDef(tileIdx)).intValue() == MQCoder.TERM_PRED_ER) {
                tmp |= OPT_PRED_TERM;
            }
	    // Error resilience segmentation symbol insertion ?
	    if(  ((Boolean)encSpec.sss.getTileDef(tileIdx)).booleanValue()) {
		tmp |= OPT_SEG_SYMBOLS;
	    }
	}
        hbuf.write(tmp);
	...
	if(mh) { // Main header
	    // Lazy coding mode ?
	    if( ((Boolean)encSpec.bms.getCompDef(compIdx)).booleanValue()) {
		tmp |= OPT_BYPASS;
	    }
	    // MQ reset after each coding pass ?
	    if( ((Boolean)encSpec.mqrs.getCompDef(compIdx)).booleanValue()) {
		tmp |= OPT_RESET_MQ;
	    }
	    // MQ termination after each arithmetically coded coding pass ?
	    if(  ((Boolean)encSpec.rts.getCompDef(compIdx)).booleanValue() ) {
		tmp |= OPT_TERM_PASS;
	    }
	    // Vertically stripe-causal context mode ?
	    if(  ((Boolean)encSpec.css.getCompDef(compIdx)).booleanValue() ) {
		tmp |= OPT_VERT_STR_CAUSAL;
	    }
            // Predictable termination ?
            if( ((Integer)encSpec.tts.getCompDef(compIdx)).intValue() == MQCoder.TERM_PRED_ER) {
                tmp |= OPT_PRED_TERM;
            }
	    // Error resilience segmentation symbol insertion ?
	    if(  ((Boolean)encSpec.sss.getCompDef(compIdx)).booleanValue()) {
		tmp |= OPT_SEG_SYMBOLS;
	    }
	} else { // Tile Header
	    if( ((Boolean)encSpec.bms.getTileCompVal(tileIdx,compIdx)).
		booleanValue()) {
		tmp |= OPT_BYPASS;
	    }
	    // MQ reset after each coding pass ?
	    if( ((Boolean)encSpec.mqrs.getTileCompVal(tileIdx,compIdx)).
		booleanValue()) {
		tmp |= OPT_RESET_MQ;
	    }
	    // MQ termination after each arithmetically coded coding pass ?
	    if(  ((Boolean)encSpec.rts.getTileCompVal(tileIdx,compIdx)).
		 booleanValue() ) {
		tmp |= OPT_TERM_PASS;
	    }
	    // Vertically stripe-causal context mode ?
	    if(  ((Boolean)encSpec.css.getTileCompVal(tileIdx,compIdx)).
		 booleanValue() ) {
		tmp |= OPT_VERT_STR_CAUSAL;
	    }
            // Predictable termination ?
            if( ((Integer)encSpec.tts.getTileCompVal(tileIdx,compIdx)).
                intValue() == MQCoder.TERM_PRED_ER) {
                tmp |= OPT_PRED_TERM;
            }
	    // Error resilience segmentation symbol insertion ?
	    if(  ((Boolean)encSpec.sss.getTileCompVal(tileIdx,compIdx)).
		 booleanValue()) {
		tmp |= OPT_SEG_SYMBOLS;
	    }
	}
    ...
    public void encodeMainHeader() throws IOException {
        ...
        // +-------------------------------+
        boolean isEresUsed = ((Integer)encSpec.tts.getDefault()).intValue() == MQCoder.TERM_PRED_ER;
        writeCOD(true,0);
        ...
        // +---------------------------------+
	for (i= 0; i<nComp; i++) {
            boolean isEresUsedinComp = ((Integer)encSpec.tts.getCompDef(i)).
                intValue() == MQCoder.TERM_PRED_ER;
	...
	// +--------------------------+
        // |    COD maker segment     |
	// +--------------------------+
        boolean isEresUsed = ((Integer)encSpec.tts.getDefault()).
            intValue() == MQCoder.TERM_PRED_ER;
        boolean isEresUsedInTile = ((Integer)encSpec.tts.getTileDef(tileIdx)).
            intValue() == MQCoder.TERM_PRED_ER;
	...
	for(int c=0; c<nComp; c++) {
            boolean isEresUsedInTileComp = ((Integer)encSpec.tts.
		...
		writeCOC(false,tileIdx,c);
                ...
                if(encSpec.wfs.isCompSpecified(c) ||
                   ...
                   (encSpec.tts.isCompSpecified(c)&&
                    ((Integer)encSpec.tts.getCompDef(c)).intValue() == MQCoder.TERM_PRED_ER)) {
                    writeCOC(false,tileIdx,c);
                }
            }
        }
