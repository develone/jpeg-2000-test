 ...
 * CVS identifier:
 *
 * $Id: AltEBCOTRateAllocator.java,v 1.1 2006/12/14 20:59:11 devel Exp vidal $
 *
 ...
 * */
public class AltEBCOTRateAllocator extends AltPostCompRateAllocator {
     ...
     * @see ProgressionType
     * */
    public AltEBCOTRateAllocator(CodedCBlkDataSrcEnc src, LayersInfo lyrs,
            AltCodestreamWriter writer,
            AltEncoderSpecs encSpec, EncoderOptions options) {
...
//        }
        ...
        pktEnc = new AltPktEncoder(src,encSpec,numPrec);
...
//    }
    ...
    public void initialize() throws IOException {
        ...
        for( int t=0 ; t<numTiles ; t++ ){
            ...
            if (((Boolean)encSpec.sops.getTileDef(t)).booleanValue()) {
                avgPktLen += Markers.SOP_LENGTH;
            }
            // Add EPH length if set
            if (((Boolean)encSpec.ephs.getTileDef(t)).booleanValue()) {
        ...
        for(int l=0; l<numLayers; l++){ //loop on layers
            ...
            for(int t=0; t<nt; t++) { //loop on tiles
                ...
                for(int c=0; c<nc; c++) { //loop on components
                    ...
                    sopUsed = ((Boolean)encSpec.sops.getTileDef(t)).
                            booleanValue();
                    // set boolean ephUsed here (EPH markers)
                    ephUsed = ((Boolean)encSpec.ephs.getTileDef(t)).
                            booleanValue();
            ...
            for(int r=rs; r<re; r++) { // loop on resolution level
                for(int c=cs; c<ce; c++) { // loop on components
                    ...
                    for(int p=0; p<nPrec; p++) { // loop on precincts
                        ...
                        // set boolean sopUsed here (SOP markers)
                        sopUsed = ((Boolean)encSpec.sops.
                                getTileDef(t)).booleanValue();
                        // set boolean ephUsed here (EPH markers)
                        ephUsed = ((Boolean)encSpec.ephs.
                                getTileDef(t)).booleanValue();
