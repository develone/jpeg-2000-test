 ...
 * CVS identifier:
 *
 * $Id: AltStdEntropyCoder.java,v 1.1 2006/12/14 20:59:11 devel Exp vidal $
 *
 ...
 * */
public class AltStdEntropyCoder extends EntropyCoder
    ...
    private AltCBlkSizeSpec cblks;
    
    /** The precinct partition specifications */
    private AltPrecinctSizeSpec pss;
...
//    }
     ...
     * @see MQCoder
     * */
    public AltStdEntropyCoder(CBlkQuantDataSrcEnc src,AltCBlkSizeSpec cblks,
            AltPrecinctSizeSpec pss,BooleanSpec bms,BooleanSpec mqrs,
            BooleanSpec rts,BooleanSpec css,BooleanSpec sss,
            AltIntegerSpec lcs,AltIntegerSpec tts) {
        super(src);
        this.cblks = cblks;
        this.pss = pss;
        this.bms = bms;
...
//                    sb.append(" ms");
//                }
//                Enumeration enumeration = idleComps.elements();
//                sb.append("\nStdEntropyCoder compressor threads wall clock "+
//                          "time:");
//                while (enumeration.hasMoreElements()) {
//                    compr = (Compressor)(enumeration.nextElement());
//                    for (c=0; c<time.length; c++) {
//                        sb.append("\n  compressor ");
    ...
    public void initTileComp(int nt, int nc) {
        ...
        for(int t=0; t<nt; t++){
            for(int c=0; c<nc; c++){
                ...
                // Bypass coding mode ?
                if( ((Boolean)bms.getTileCompVal(t,c)).booleanValue()) {
                    opts[t][c] |= OPT_BYPASS;
                }
                // MQ reset after each coding pass ?
                if(((Boolean)mqrs.getTileCompVal(t,c)).booleanValue()) {
                    opts[t][c] |= OPT_RESET_MQ;
                }
                // MQ termination after each arithmetically coded coding pass ?
                if(((Boolean)rts.getTileCompVal(t,c)).booleanValue()) {
                    opts[t][c] |= OPT_TERM_PASS;
                }
                // Vertically stripe-causal context mode ?
                if(((Boolean)css.getTileCompVal(t,c)).booleanValue()) {
                    opts[t][c] |= OPT_VERT_STR_CAUSAL;
                }
                // Error resilience segmentation symbol insertion ?
                if(((Boolean)sss.getTileCompVal(t,c)).booleanValue()) {
                    opts[t][c] |= OPT_SEG_SYMBOLS;
                }
                
                // Set length calculation type of the MQ coder
                int lCalcType = ((Integer)lcs.getTileCompVal(t,c)).intValue();
                lenCalc[t][c] = lCalcType;               
                
                // Set termination type of MQ coder
                int termType = ((Integer)tts.getTileCompVal(t,c)).intValue();
                tType[t][c] = termType;          
