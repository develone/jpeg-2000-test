 ...
 * CVS identifier:
 *
 * $Id: AltCBlkSizeSpec.java,v 1.1 2006/12/14 20:58:57 devel Exp vidal $
 *
 * Class:                   CBlkSizeSpec
 *
 ...
 * */
public class AltCBlkSizeSpec extends ModuleSpec {
     ...
     * */
    public AltCBlkSizeSpec(int nt, int nc, byte type) {
        super(nt, nc, type);
    }
     ...
     * */
    public AltCBlkSizeSpec(int nt, int nc, byte type, EncoderOptions options) {
        super(nt, nc, type);
        
        int cblksizx = options.cblksizx;
        int cblksizy = options.cblksizy;
        
        Integer dim[] = new Integer[2];
        
        dim[0] = Integer.valueOf(cblksizx);
        // Check that width is not >
        // StdEntropyCoderOptions.MAX_CB_DIM
        if( cblksizx>StdEntropyCoderOptions.MAX_CB_DIM ){
            String errMsg = "'"+optName+"' option : the code-block's "+
        ...
        if( cblksizx<StdEntropyCoderOptions.MIN_CB_DIM ){
            String errMsg = "'"+optName+"' option : the code-block's "+
        ...
        if ( cblksizx !=
                (1<<MathUtil.log2(cblksizx)) ) {
            String errMsg = "'"+optName+"' option : the code-block's "+
                    "width must be a power of 2";
            throw new IllegalArgumentException(errMsg);
        }
        
        dim[1] = Integer.valueOf(cblksizy);
        // Check that height is not >
        // StdEntropyCoderOptions.MAX_CB_DIM
        if ( cblksizy>StdEntropyCoderOptions.MAX_CB_DIM ){
            String errMsg = "'"+optName+"' option : the code-block's "+
        ...
        // Check that height is not <
        // StdEntropyCoderOptions.MIN_CB_DIM
        if ( cblksizy <StdEntropyCoderOptions.MIN_CB_DIM ){
            String errMsg = "'"+optName+"' option : the code-block's "+
        ...
        if ( cblksizx*cblksizy >
                StdEntropyCoderOptions.MAX_CB_AREA ) {
            String errMsg = "'"+optName+"' option : The "+
        ...
        if ( cblksizx > maxCBlkWidth ) {
            maxCBlkWidth = cblksizx;
        }
        
        if ( cblksizy > maxCBlkHeight ) {
            maxCBlkHeight = cblksizy;
        }
