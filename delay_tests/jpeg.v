// File: jpeg.v
// Generated by MyHDL 0.9dev
// Date: Tue Sep 30 13:34:17 2014


`timescale 1ns/10ps

module jpeg (
    clk_fast,
    left_s,
    leftDelDut_s,
    right_s,
    sam_s,
    res_s,
    updated_jpeg_s,
    even_odd_s,
    fwd_inv_s,
    updated_s,
    noupdate_s,
    sigDelayed_s
);


input clk_fast;
input signed [15:0] left_s;
input signed [15:0] leftDelDut_s;
input signed [15:0] right_s;
input signed [15:0] sam_s;
output signed [15:0] res_s;
reg signed [15:0] res_s;
output updated_jpeg_s;
reg updated_jpeg_s;
input even_odd_s;
input fwd_inv_s;
input updated_s;
output noupdate_s;
reg noupdate_s;
input sigDelayed_s;






always @(posedge clk_fast) begin: JPEG_HDL
    if (updated_s) begin
        if (even_odd_s) begin
            if (fwd_inv_s) begin
                if (sigDelayed_s) begin
                    res_s <= (sam_s - ($signed(leftDelDut_s >>> 1) + $signed(right_s >>> 1)));
                end
                else begin
                    res_s <= (sam_s - ($signed(left_s >>> 1) + $signed(right_s >>> 1)));
                end
            end
            else begin
                if (sigDelayed_s) begin
                    res_s <= (sam_s + ($signed(leftDelDut_s >>> 1) + $signed(right_s >>> 1)));
                end
                else begin
                    res_s <= (sam_s + ($signed(left_s >>> 1) + $signed(right_s >>> 1)));
                end
            end
        end
        else begin
            if (fwd_inv_s) begin
                if (sigDelayed_s) begin
                    res_s <= (sam_s + $signed(((leftDelDut_s + right_s) + 2) >>> 2));
                end
                else begin
                    res_s <= (sam_s + $signed(((left_s + right_s) + 2) >>> 2));
                end
            end
            else begin
                if (sigDelayed_s) begin
                    res_s <= (sam_s - $signed(((leftDelDut_s + right_s) + 2) >>> 2));
                end
                else begin
                    res_s <= (sam_s - $signed(((left_s + right_s) + 2) >>> 2));
                end
            end
        end
        updated_jpeg_s <= 1;
    end
    else begin
        noupdate_s <= 1;
    end
end

endmodule
