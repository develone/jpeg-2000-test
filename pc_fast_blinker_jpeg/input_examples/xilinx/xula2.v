...
assign data_to_host7 = ((z14 << 16) | z14);
...
always @(posedge clock) begin: XULA2_BEH_MY_RET_REG
    if (memmap_read) begin
        if ((memmap_mem_addr == 70)) begin
            memmap_read_data <= data_to_host0;
        end
        if ((memmap_mem_addr == 74)) begin
            memmap_read_data <= data_to_host1;
        end
        if ((memmap_mem_addr == 78)) begin
            memmap_read_data <= data_to_host2;
        end
        if ((memmap_mem_addr == 82)) begin
            memmap_read_data <= data_to_host3;
        end
        if ((memmap_mem_addr == 86)) begin
            memmap_read_data <= data_to_host4;
        end
        if ((memmap_mem_addr == 90)) begin
            memmap_read_data <= data_to_host5;
        end
        if ((memmap_mem_addr == 94)) begin
            memmap_read_data <= data_to_host6;
        end
        if ((memmap_mem_addr == 98)) begin
