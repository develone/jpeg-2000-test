...
wire fifobus_write;
reg fifobus_read_valid;
reg reset;
reg [31:0] reset_dly_cnt;
wire fifobus_read;
wire [7:0] fifobus_write_data;
...
always @(posedge clock, posedge reset) begin: XULA2_SPI_INST_FIFO_TX_INST_RTL_FIFO
    if (reset == 1) begin
        spi_inst_itx_full <= 0;
...
//        MISO  ......|.0.....|.1.....|.2.....|.3.....|.4.....|.5.....|.6.....|.7.....|.0...
always @(posedge clock, posedge reset) begin: XULA2_SPI_INST_RTL_STATE_AND_MORE
    if (reset == 1) begin
        spi_inst_itx_read <= 0;
        spi_inst_x_sck <= 0;
        spi_inst_treg <= 0;
...
always @(fifobus_write, spi_inst_rreg, spi_inst_irx_empty, spi_inst_cso_rx_read, spi_inst_cso_bypass_fifo, spi_inst_itx_full, spi_inst_irx_read_data, spi_inst_irx_full, spi_inst_itx_empty, fifobus_read, spi_inst_irx_read_valid, spi_inst_cso_tx_write, fifobus_write_data, spi_inst_cso_tx_byte) begin: XULA2_SPI_INST_RTL_FIFO_SEL
    if (spi_inst_cso_bypass_fifo) begin
        ...
        spi_inst_fifobus_full = spi_inst_itx_full;
        ...
        fifobus_read_valid = spi_inst_irx_read_valid;
...
always @(posedge clock, posedge reset) begin: XULA2_SPI_INST_FIFO_RX_INST_RTL_FIFO
    if (reset == 1) begin
        spi_inst_irx_full <= 0;
...
assign fifobus_write_data = fifobus_read_data;
assign fifobus_write = fifobus_read_valid;
assign fifobus_read = (!fifobus_empty);
