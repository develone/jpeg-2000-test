...
t_State = enum('INIT', 'ODD_SAMPLES', 'EVEN_SAMPLES', 'WR_DATA', 'INTERLACE', 'DONE', encoding="one_hot")
	...
	def file_rd():
		if (rst_file_in == 0):
			...
			if (rst == 1):
				...
				wr_s.next = 0
	return file_rd
def muxaddr(addr_r, addr_r1, addr_r2, muxsel):
	@always_comb
	def muxLogic():
		addr_r.next = addr_r1
		if (muxsel == 1):
...
def RamCtrl(addr_r, addr_x, state, even_odd_r, even_odd_x):
    @always_comb
    def FSM():
        even_odd_x.next <= 0
        if state == t_State.INIT:
            addr_x.next = 1
            state.next <= t_State.ODD_SAMPLES
        elif state == t_State.ODD_SAMPLES:
            if addr_r == 1:
                addr_x.next = 2
                state.next <= t_State.EVEN_SAMPLES
        elif state == t_State.EVEN_SAMPLES:
            addr_x.next = 2
...
#def xess_jpeg_top(clk_fast, addr_r, addr_x, addr_r1, addr_r2, muxsel, dataToRam_r, dataToRam_x, sig_in, noupdate_s, res_s, jp_lf, jp_sa ,jp_rh, jp_flgs, reset_col, rdy, addr_not_reached, offset, dataFromRam_s, state, even_odd_r, even_odd_x):
def xess_jpeg_top(clk_fast, addr_r, addr_x, addr_r1, addr_r2, muxsel, dataToRam_r, dataToRam_x, sig_in, noupdate_s, res_s, jp_lf, jp_sa ,jp_rh, jp_flgs, reset_col, rdy, addr_not_reached, offset, dataFromRam_s):
    ...
    instance_2 = muxaddr(addr_r, addr_r1, addr_r2, muxsel)
    ...
    #instance_6 = RamCtrl(addr_r, addr_x, state, even_odd_r, even_odd_x)
    instance_7 = jpegfsmupdate(clk_fast, addr_r1, addr_x, dataToRam_r, dataToRam_x)
    ...
    #return instance_2, instance_3, instance_4, instance_5, instance_6, instance_7
    return instance_2, instance_3, instance_4, instance_5, instance_7
 
#toVHDL(RamCtrl, addr_r, addr_x, state, even_odd_r, even_odd_x)
...
#toVHDL(xess_jpeg_top, clk_fast, addr_r, addr_x, addr_r1, addr_r2, muxsel, dataToRam_r, dataToRam_x, sig_in, noupdate_s, res_s, jp_lf, jp_sa ,jp_rh, jp_flgs, reset_col, rdy, addr_not_reached, offset, dataFromRam_s, state, even_odd_r, even_odd_x)
...
toVHDL(xess_jpeg_top, clk_fast, addr_r, addr_x, addr_r1, addr_r2, muxsel, dataToRam_r, dataToRam_x, sig_in, noupdate_s, res_s, jp_lf, jp_sa ,jp_rh, jp_flgs, reset_col, rdy, addr_not_reached, offset, dataFromRam_s)
