...
JPEGDZ = 9
"""
dout_rom = Signal(intbv(0)[31:])
addr_rom_r = Signal(intbv(0)[ROM_ADDR:])
addr_rom_x = Signal(intbv(0)[ROM_ADDR:])

def rom(dout_rom, addr_rom_x, CONTENT):
    

    @always_comb
    def read():
        dout_rom.next = CONTENT[int(addr_rom_x)]
...
right_s 9 bits sam_s left_s 9 bits
sig0_in_x 31 bits
...
sig_in = Signal(intbv(0)[31:])
"""
sig_in_r = Signal(intbv(0)[31:])
...
def JPEGFSM(state_r, state_x, addr_rom_r, addr_rom_x, sig_in_r, sig_in_x, dout_rom):
    @always_comb
    def FSM():
        addr_rom_x.next = addr_rom_r
        sig_in_x.next = sig_in_r
        state_x.next = state_r
        if state_r == t_State.INIT:
            addr_rom_x.next = 0
            sig_in_x.next = 0
            state_x.next = t_State.READ_ROM
        elif state_r == t_State.READ_ROM:
            if noupdate_s == YES:
                if (addr_rom_r <= 130046):
                    sig_in_x.next = dout_rom
...
def jpegfsmupdate(clk_fast,
                  state_r, state_x, addr_rom_r, addr_rom_x,
                  sig_in_r, sig_in_x
                  ):
    @always(clk_fast.posedge)
    def fsmupdate():
...
def xess_jpeg_para(clk_fast, state_r, state_x, sig_in_r, sig_in_x, noupdate_s, res_s, dout_rom, addr_rom_r, addr_rom_x, cc):
    ...
    instance_4 = rom(dout_rom, addr_rom_x, cc)
...
def testbench( clk_fast, sig0_in_x, noupdate0_s, res0_s,
    ...
    def stimulus():
        ...
        for ii in range(0, 16381, 8):
            ...
            sig3_in_x.next = cc[ii+3]
            sig4_in_x.next = cc[ii+4]
