...
jp_sa = Signal(intbv(0)[JPEGDZ:])
jp_rh = Signal(intbv(0)[JPEGDZ:])
...
def RamCtrl(addr0_r, addr0_x,
            addr1_r, addr1_x, state_r, state_x,
...
def jpegfsmupdate(clk_fast,
                  addr0_r, addr0_x, addr1_r, addr1_x, state_r,
                  state_x,
                  dataToRam0_r, dataToRam0_x, dataToRam1_r, dataToRam1_x,
...
def jpeg_process(clk_fast, sig_in,  noupdate_s, res_s, res_u):
    left_s = sig_in(8,0)
    sam_s = sig_in(16,8)
    right_s = sig_in(24,16)
    even_odd_s = sig_in(23)
    fwd_inv_s = sig_in(24)
    updated_s = sig_in(25)
    dum_s = sig_in(27)
    @always(clk_fast.posedge)
...
def jpegram2sig(jp_lf, jp_sa ,jp_rh, jp_flgs, rdy, addr_not_reached,  sig_in):
    """Combines 3 8 bit plus 4 flags into single value """
    @always_comb
    def ram2sig():
        if rdy:
            if addr_not_reached:
...
def ramres(dout_res_r, din_res_r, addr_res_r, we_res, clk_fast, depth=256):
    ...
    return write, read
def xess_jpeg_para(clk_fast, sig_in, noupdate_s, res_s, res_u,
                  jp_lf, jp_sa ,jp_rh, jp_flgs, rdy, addr_not_reached):
    instance_1 = jpeg_process(clk_fast, sig_in,  noupdate_s, res_s, res_u)
...
def xess_jpeg_top(clk_fast,
    ...
    instance_6 = RamCtrl(
            ...
            dout_rom, addr_rom_r, addr_rom_x, rdy, res_u, noupdate_s,
            index1_r, index2_r, index3_r, index1_x, index2_x, index3_x) 
    instance_7 = jpegfsmupdate(clk_fast,
                               addr0_r, addr0_x, addr1_r, addr1_x,
                               state_r, state_x,
                               dataToRam0_r, dataToRam0_x,
                               dataToRam1_r, dataToRam1_x,
    ...
    return instance_1, instance_6, instance_7, instance_8
     

toVHDL(xess_jpeg_para, clk_fast, sig_in, noupdate_s, res_s, res_u,
                  jp_lf, jp_sa ,jp_rh, jp_flgs, rdy, addr_not_reached) 
