...
def del_test(clk_fast, left_s, delay_s, signed_res_s):
    @always(clk_fast.posedge)
    def hdl():
        signed_res_s.next = left_s + 1
    return hdl
toVerilog(del_test, clk_fast, left_s, delay_s, signed_res_s)
toVHDL(del_test, clk_fast, left_s, delay_s, signed_res_s)