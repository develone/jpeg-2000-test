...
def fwt97(s, width, height):
    ...
    for col in range(width): # Do the 1D transform on all cols:
        ...
        for row in range(2, height, 2):
			#print (s[row][col])
			ll = (s[row][col] - ((int(s[row-1][col])>>1) + (int(s[row+1][col])>>1)))
			lift = jpeg.Exec(int((s[row+1][col])), int((s[row-1][col])), int((s[row][col])), even_odd, fwd_inv)
			s[row][col] = lift.int
        ...
        for row in range(1, height-1, 2):
			ll = (s[row][col] + ((int(s[row-1][col]) + int(s[row+1][col]) + 2)>>2))
			lift = jpeg.Exec(int((s[row+1][col])), int((s[row-1][col])), int((s[row][col])), even_odd, fwd_inv)
			s[row][col] = lift.int
			print '%3d %3d %5d %5d %1d %1d %5d %5d' % (col, row, (s[row+1][col]), (s[row-1][col]), even_odd, fwd_inv, (s[row][col]), ll)
