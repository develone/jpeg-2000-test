...
def pc_read(clk, data_in, toLift_Step, we_in, addr_in, muxsel_i ):
    @always(clk.posedge)
    def readlogic():
        if( muxsel_i == 1):
            if (addr_in <= 256):
...
def detect_data_from_pc(status_i, datapush):
    ...
    def rtl():
        if (datapush == 0):
            ...
            status_i.next = 0
    return rtl
def tb(clk, data_in, toLift_Step, we_in, addr_in, muxsel_i):
    ...
    return instances()
#tb_fsm = traceSignals( tb, clk, data_in, toLift_Step, we_in, addr_in, muxsel_i )
#sim = Simulation(tb_fsm)
#sim.run()
toVHDL(pc_read, clk, data_in, toLift_Step, we_in, addr_in, muxsel_i) 
